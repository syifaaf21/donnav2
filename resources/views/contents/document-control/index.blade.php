...
<div class="container mx-auto my-2 px-4">
    ...
    @php
        ...
        $obsoleteDocuments = $documentsMapping->filter(fn($d) => optional($d->status)->name === 'Obsolete')->count();
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                @forelse ($groupedDocuments as $department => $mappings)
                    <div class="border border-gray-100 rounded-xl overflow-hidden">
                        <button type="button"
                            ...
                            <svg class="w-4 h-4 text-gray-500 transition-transform transform rotate-0" data-rotate-for="panel-{{ $loop->index }}"
                                        ...
                                        <div class="flex items-start justify-between gap-3">
                                            <div class="flex-1">
                                                ...
                                                <div class="mt-1 text-xs text-gray-500 space-y-0.5">
                                                    ...
                                                    <p>Note: {{ $mapping->notes ?? '-' }}</p>
                                                    <p>Valid until: {{ $mapping->obsolete_date ? \Carbon\Carbon::parse($mapping->obsolete_date)->format('d M Y') : '-' }}</p>
                                                </div>
                                            </div>
                                                    ...
                                                    @foreach ($mapping->files as $file)
                                                        <button type="button"
                                                            ...
                                                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                                                <path d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7l-4-4H4z"></path>
                                                            </svg>
                                                            View {{ $loop->iteration }}
                                                        </button>
                                                    @endforeach
        ...
        <div class="lg:col-span-8 bg-white border border-gray-100 rounded-2xl shadow-sm p-0 flex flex-col">
            <div class="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gray-50 rounded-t-2xl">
                <div class="flex items-center gap-3">
                    <svg class="w-5 h-5 text-sky-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A2 2 0 0122 9.618V16a2 2 0 01-2 2H6a2 2 0 01-2-2V6.382a2 2 0 01.447-1.894L9 10m6 0V4"></path>
                    </svg>
                ...
                <div id="actionButtons" class="hidden flex items-center gap-2">
                    <button class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-yellow-200 text-yellow-700 hover:bg-yellow-50"
                        id="reviseBtn" data-bs-toggle="modal" data-bs-target="#reviseModalDynamic">
                        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v6h6"></path></svg>
                        Revise
                    </button>

                    <form id="approveForm" method="POST" class="inline" action="#">
                        @csrf
                        <button type="submit" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-green-200 text-green-700 hover:bg-green-50">
                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                            Approve
                        </button>
                    </form>
                    ...
                    <form id="rejectForm" method="POST" class="inline" action="#">
                        @csrf
                        <button type="submit" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-red-200 text-red-700 hover:bg-red-50">
                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                            Reject
                        </button>
                    </form>
                </div>
            ...
            <div id="previewContainer" class="flex-1 p-4 flex items-center justify-center" style="min-height: calc(85vh - 60px);">
