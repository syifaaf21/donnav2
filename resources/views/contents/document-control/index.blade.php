                                    ...
                                    @foreach ($mappings as $mapping)
                                        <div class="card mb-2 border-0 shadow-sm">
                                            <div class="card-body py-2">
                                                ...
                                                <p class="text-muted small mb-1">Last Update: {{ $mapping->updated_at }}
                                                </p>
                                                ...
                                                @foreach ($mapping->files as $file)
                                                    <button type="button"
                                                        ...
                                                        data-files='@json(
                                                            $mapping->files->map(function ($f) {
                                                                return ['id' => $f->id, 'name' => basename($f->file_path), 'url' => asset('storage/' . $f->file_path)];
                                                            }))'>
        ...
        <script>
            (function() {
                ...
                reviseModalEl.addEventListener('show.bs.modal', (event) => {
                    ...
                    if ((!files || files.length === 0) && document.querySelector(
                            `.view-file-btn[data-docid="${docId}"]`)) {
                        const sampleBtn = document.querySelector(`.view-file-btn[data-docid="${docId}"]`);
                        try {
                            files = JSON.parse(sampleBtn.dataset.files || '[]');
                        } catch (e) {
                            files = [];
                        }
                    }
