    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function populateReviseModal(btn) {
                // Filter hanya file yang aktif
const files = (JSON.parse(btn.dataset.files || '[]')).filter(f => f.is_active === 1);
                ...
                <div class="mt-2 hidden file-input-wrapper">
                    <input type="file" name="revision_files[]" data-old-file-id="${f.id}">
                </div>
            ...
            `;
                    ...
                    if (replaceBtn) {
                        replaceBtn.addEventListener('click', () => {
                            ...
                            // Pastikan hidden input old_file_ids[] nilainya tetap f.id
                            const hidden = wrapper.querySelector('input[name="old_file_ids[]"]');
                            if (hidden) hidden.value = input.dataset.oldFileId;
                        });
                    }
                });
            }
