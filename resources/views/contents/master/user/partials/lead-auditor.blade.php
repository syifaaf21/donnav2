            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($users as $user)
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <td class="px-4 py-2 border-r border-gray-200">
                            @php
                                $leadAuditorAuditTypes = $user->userAuditTypes
                                    ->filter(fn($uat) => $uat->userRole && $uat->userRole->role &&
                                        $uat->userRole->role->name === 'Lead Auditor')
                                    ->map(fn($uat) => optional($uat->audit)->name)
                                    ->filter()
                                    ->unique()
                                    ->values();
                            @endphp
                            {{ $leadAuditorAuditTypes->join(', ') ?: '-' }}
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center text-gray-500 py-4">No lead auditors found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
