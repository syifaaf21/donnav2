    ...
    <div class="space-y-6">
            ...
            <table class="w-full border border-gray-200 text-sm mt-2">
                ...
                <template x-for="i in 4">
                    <tr>
                        ...
                        <td class="border border-gray-200"><input type="text" class="w-full p-1 border-none"
                                x-model="form['corrective_'+i+'_activity']"></td>
                        <td class="border border-gray-200"><input type="text" class="w-full p-1 border-none"
                                x-model="form['corrective_'+i+'_pic']"></td>
                        <td class="border border-gray-200"><input type="date" class="w-full p-1 border-none"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-2">
                    ...
                    <div class="flex gap-6">
                        <label><input type="radio" name="yokoten" value="1" x-model="form.yokoten"> Yes</label>
                        <label><input type="radio" name="yokoten" value="0" x-model="form.yokoten"> No</label>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white p-6 mt-6 border border-gray-200 rounded-lg shadow space-y-6">
            ...
            <div class="flex items-center justify-between">
                ...
                <!-- Save Action Button -->
                {{-- <button type="button" onclick="updateAuditeeAction()"
                    class="flex ml-auto bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                    Save Auditee Action
                </button> --}}
            </div>
        </div>
...
<script>
    ...
    async function updateAuditeeAction(isApprove = false) {
        ...
        // -----------------------------
        for (let i = 1; i <= 5; i++) {
            formData.append(`why_${i}_mengapa`, document.querySelector(`input[x-model="form['why_${i}_mengapa']"]`)
                ?.value || '');
            formData.append(`cause_${i}_karena`, document.querySelector(
                `input[x-model="form['cause_${i}_karena']"]`)?.value || '');
        }
        ...
        // -----------------------------
        for (let i = 1; i <= 4; i++) {
            formData.append(`corrective_${i}_activity`,
                document.querySelector(`input[x-model="form['corrective_${i}_activity']"]`)?.value || '');

            formData.append(`corrective_${i}_pic`,
                document.querySelector(`input[x-model="form['corrective_${i}_pic']"]`)?.value || '');

            formData.append(`corrective_${i}_planning`,
                document.querySelector(`input[x-model="form['corrective_${i}_planning']"]`)?.value || '');

            formData.append(`corrective_${i}_actual`,
                document.querySelector(`input[x-model="form['corrective_${i}_actual']"]`)?.value || '');
        }
        ...
        // -----------------------------
        for (let i = 1; i <= 4; i++) {
            formData.append(`preventive_${i}_activity`,
                document.querySelector(`input[x-model="form['preventive_${i}_activity']"]`)?.value || '');

            formData.append(`preventive_${i}_pic`,
                document.querySelector(`input[x-model="form['preventive_${i}_pic']"]`)?.value || '');

            formData.append(`preventive_${i}_planning`,
                document.querySelector(`input[x-model="form['preventive_${i}_planning']"]`)?.value || '');

            formData.append(`preventive_${i}_actual`,
                document.querySelector(`input[x-model="form['preventive_${i}_actual']"]`)?.value || '');
        }
